...
export default function ProductPreviews() {
  ...
  useEffect(() => {
    let active = true;
    axiosClient.get("/paypal/config").then(({ data }) => { if (active) setPaypal(data); })
      .catch(() => { if (active) setPaypal({ enabled: false, mode: "sandbox", piSignedIn: false }); });
    return () => { active = false; };
  }, [isAuthenticated]);
  ...
  return <main className="storefront">
    <section style={{ padding: 24, maxWidth: 960, margin: "auto" }}>
      ...
      {!isAuthenticated && !paypal.piSignedIn && <p>{!paypal.enabled ? "PayPal checkout is not available yet." : paypal.mode === "sandbox" ? "PayPal is in sandbox testing mode. No real money is charged." : "Use PayPal for USD checkout. Keep the private recovery link provided after checkout for future downloads."} Pi purchases are available through Pi Browser.</p>}
    ...
    {products.map(product => <section key={product.id} id={product.previewSlug} style={{ scrollMarginTop: 24 }}>
      <ProductCard {...product} showPayments showPayPal={!isAuthenticated && !paypal.piSignedIn}
